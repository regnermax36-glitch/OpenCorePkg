...
## Description
This DXE driver tries to register keyboard shortcut (F10) handler for all text input devices. The handler tries to find a writable FS preferring MaxRegner root if available, finds primary GOP device, takes screenshot from it and saves the result as PNG files on that writable FS.
...
- Install the driver via MaxRegner
